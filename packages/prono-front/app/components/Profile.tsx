import { currentUser, groups, predictions, availableBadges } from "../data/mockData";
...
export function Profile({ onBack }: ProfileProps) {
  ...
  const userPredictions = predictions.filter(p => p.user_id === currentUser.id);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
        ...
        <div className="bg-white border border-[#E5E4E1] p-6">
          ...
          <p className="text-sm text-gray-600 mt-1">
            {correctPredictions.length}/{completedPredictions.length} pronostics corrects
          </p>
        </div>
      ...
      <div className="mb-8 sm:mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {userGroups.map(group => (
            <div key={group.id} className="bg-white border border-[#E5E4E1] p-6">

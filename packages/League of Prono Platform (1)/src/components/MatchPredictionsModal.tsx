import { Match, Prediction, users } from "../data/mockData";
...
export function MatchPredictionsModal({ match, predictions, isOpen, onClose }: MatchPredictionsModalProps) {
  if (!match) return null;

  // Compter les pronostics par équipe
  const teamAPredictions = predictions.filter(p => p.predicted_winner === "team_a").length;
  const teamBPredictions = predictions.filter(p => p.predicted_winner === "team_b").length;
  const totalPredictions = predictions.length;

  const teamAPercentage = totalPredictions > 0 ? Math.round((teamAPredictions / totalPredictions) * 100) : 0;
  const teamBPercentage = totalPredictions > 0 ? Math.round((teamBPredictions / totalPredictions) * 100) : 0;
      ...
      <DialogContent className="max-w-3xl bg-white border-[#E5E4E1]">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-600">
            {match.team_a} vs {match.team_b} • {totalPredictions} pronostic{totalPredictions > 1 ? 's' : ''}
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {predictions.length === 0 ? (
                <div className="bg-white border border-[#E5E4E1] p-8 text-center">
                  ...
                  <p className="text-gray-600 text-sm">Aucun pronostic pour ce match</p>
                </div>
              ) : (
                predictions.map(prediction => {
                  ...
                  const predictedTeam = prediction.predicted_winner === "team_a" ? match.team_a : match.team_b;
                  const hasScore = prediction.predicted_score_a !== undefined && prediction.predicted_score_b !== undefined;
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          } text-lg`} style={{ fontWeight: 600 }}>
                            {user.name.charAt(0).toUpperCase()}
                          </div>
                          <div>
                            <div style={{ fontWeight: 600 }}>{user.name}</div>
                            <div className="text-sm text-gray-600">
                              Prédit : <span style={{ fontWeight: 600 }}>{predictedTeam}</span>
                              {hasScore && (

      ...
      <Tabs defaultValue="friends" className="w-full">
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4 mb-8 bg-[#F5F4F1] border border-[#E5E4E1]">
          ...
          >
            ...
            <span className="hidden sm:inline">Demandes ({friendRequests.length})</span>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#548CB4] bg-opacity-10 flex items-center justify-center text-[#548CB4] text-lg sm:text-xl" style={{ fontWeight: 600 }}>
                        {friend.name.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <h4 className="mb-1">{friend.name}</h4>
                        <p className="text-sm text-gray-600 break-all">{friend.email}</p>
                        {friend.common_groups !== undefined && friend.common_groups > 0 && (
                          <p className="text-xs text-[#548CB4] mt-1">
                            {friend.common_groups} groupe{friend.common_groups > 1 ? 's' : ''} en commun
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between sm:gap-4">
                      <div>
                        <div className="flex items-center gap-2 text-[#C4A15B]">
                          ...
                          <span style={{ fontWeight: 600 }}>{friend.total_points} pts</span>
                        </div>
                      </div>
                      <Button
                        variant="outline"
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-[#548CB4] bg-opacity-10 flex items-center justify-center text-[#548CB4] text-xl" style={{ fontWeight: 600 }}>
                        {request.name.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <h4 className="mb-1">{request.name}</h4>
                        <p className="text-sm text-gray-600">{request.email}</p>
                        {request.common_groups !== undefined && request.common_groups > 0 && (
                          <p className="text-xs text-[#548CB4] mt-1">
                            {request.common_groups} groupe{request.common_groups > 1 ? 's' : ''} en commun
                          </p>
                        )}
                      </div>
                    </div>
            ...
            <div className="space-y-3">
              {sentRequests.map(request => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-gray-100 flex items-center justify-center text-gray-600 text-xl" style={{ fontWeight: 600 }}>
                        {request.name.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <h4 className="mb-1">{request.name}</h4>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white border border-[#E5E4E1] p-8">
              ...
              <p className="text-gray-600 mb-6">
                Entrez l'adresse email d'un utilisateur pour lui envoyer une demande d'ami
              </p>
              ...
              <div className="space-y-3">
                {friendSuggestions.map(suggestion => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-[#C4A15B] bg-opacity-10 flex items-center justify-center text-[#C4A15B] text-xl" style={{ fontWeight: 600 }}>
                          {suggestion.name.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <h4 className="mb-1">{suggestion.name}</h4>
                          <p className="text-sm text-gray-600">{suggestion.email}</p>
                          {suggestion.common_groups !== undefined && suggestion.common_groups > 0 && (
                            <p className="text-xs text-[#548CB4] mt-1">
                              {suggestion.common_groups} groupe{suggestion.common_groups > 1 ? 's' : ''} en commun
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <div className="flex items-center gap-2 text-[#C4A15B]">
                            <Trophy className="w-4 h-4" />
                            <span style={{ fontWeight: 600 }}>{suggestion.total_points} pts</span>
                          </div>
                        </div>
                        <Button

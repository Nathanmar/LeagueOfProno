...
import { Trophy, Users, ShoppingBag, Home, Menu, X, User, LogOut } from "lucide-react";
...
export function Header({ onNavigateToProfile, onNavigateToFriends, onNavigateToDashboard, onNavigateToShop, onLogout, currentView }: HeaderProps) {
  const [isOpen, setIsOpen] = useState(false);
  
  const navItems = [
    { id: "dashboard", label: "Accueil", icon: Home, onClick: onNavigateToDashboard },
    { id: "friends", label: "Amis", icon: Users, onClick: onNavigateToFriends },
    { id: "shop", label: "Boutique", icon: ShoppingBag, onClick: onNavigateToShop },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          >
            ...
            <span className="text-lg sm:text-xl hidden sm:block" style={{ fontWeight: 700 }}>League of Prono</span>
          </button>
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navItems.map((item) => {
              ...
              const isActive = currentView === item.id || 
                              (item.id === "dashboard" && (currentView === "dashboard" || currentView === "group"));
            ...
            <DropdownMenuTrigger asChild>
              ...
              >
                <div className="text-right">
                  <div style={{ fontWeight: 600 }} className="text-sm">{currentUser.name}</div>
